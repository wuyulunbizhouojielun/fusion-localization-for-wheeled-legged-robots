...
ind_sentry = 0  # 功能同ind，但是单独负责给哨兵通信
ind_our_coor = 0
Id_red = 1
Id_blue = 101
Id_red_sentry = 1  # 防止干扰小地图，发给哨兵循环
Id_blue_sentry = 101
...
Game_mark_data = Game_data_define.mark_data_t()   # 和无人机通信，发送标记进度
...
def read(ser):
    ...
    while True:
        ...
        if buffercnt == 13 and cmdID == 0x0102:
            if offical_Judge_Handler.myVerify_CRC16_Check_Sum(id(buffer), 13):
                Referee_supply_projectile_action()
                buffercnt = 0
                if buffer[buffercnt] == 0xa5:
                    buffercnt = 1
                continue
...
class UART_passer(object):
    ...
    # 5.13 为了方便赛后查看，将机器人标记进度数据保存，在主函数中写入logger info 范围 0-120
    mark_hero = Game_mark_data.mark_hero_progress
    mark_engineer = Game_mark_data.mark_engineer_progress
    mark_standard_3 = Game_mark_data.mark_standard_3_progress
    mark_standard_4 = Game_mark_data.mark_standard_4_progress
    mark_standard_5 = Game_mark_data.mark_standard_5_progress
    mark_sentry = Game_mark_data.mark_sentry_progress
...
def Receive_radar_info():
    UART_passer.radar_vulnerable_chances = buffer[7] & 0x03 # bit 1 和 bit 0
...
# 哨兵通信            
def ControlLoop_red_sentry():
    '''
    循环红色车辆id
    '''
    global Id_red_sentry
    if Id_red_sentry == 7:
        Id_red_sentry = 1
    else:
        if Id_red_sentry == 5:
            Id_red_sentry = Id_red_sentry + 2  # 跳过空中机器人
        else:
            Id_red_sentry = Id_red_sentry + 1

def ControlLoop_blue_sentry():
    '''
    循环蓝色车辆id
    '''
    global Id_blue_sentry
    if Id_blue_sentry == 107:
        Id_blue_sentry = 101
    else:
        if Id_blue_sentry == 105:
            Id_blue_sentry = Id_blue_sentry + 2
        else:
            Id_blue_sentry = Id_blue_sentry + 1
...
# 4.23
def radar_trigger_double_vulnerable(ser):
...
def radar_transmit_double_vulnerable(dataID, ReceiverId, data, ser):
    ...
    offical_Judge_Handler.Append_CRC16_Check_Sum(id(buffer), 7 + 9)  # 整包校验

    buffer_tmp_array = [0]
    buffer_tmp_array *= 9 + 7
...
# 发送车间通信车辆坐标
# 5.8 因为裁判系统发送相当于单独一个线程，所以自己会循环发送对应的坐标，速度上应该远远高于主线程
...
# 和哨兵车间通信
def Transmit_with_sentry(ser):
    
    global ind_sentry
    location = UART_passer.get_position()[ind_sentry] # 得到当前帧位置（来源于主循环中检测出的敌方单位坐标）
    x,y = location
    ...
    if reverse:
        if send_enable: # 置零便不发送处理
            print('for sentry x: {} y: {}' .format(x, y))
            Referee_Transmit_Sentry(dataID, receiverId, 15, np.float32(x), np.float32(y), Id_red_sentry, ser)
            time.sleep(0.1)
        ControlLoop_red_sentry() # 循环红色车编号 1-5 哨兵 7
    else:
        if send_enable:
            print('for sentry x: {} y: {}' .format(x, y))
            Referee_Transmit_Sentry(dataID, receiverId, 15, np.float32(x), np.float32(y), Id_blue_sentry, ser) 
            time.sleep(0.1)
        ControlLoop_blue_sentry() # 循环红色车编号 101-105 哨兵 107

    if send_enable:
        UART_passer.loop_send_sentry += 1 # 在一轮循环中发送次数

    # 4.24 加入哨兵，循环次数应该是+1
    if ind_sentry == 5:
        if  UART_passer.loop_send_sentry == 0: # 若一轮循环结束，没有发送一次，则sleep 0.1s来保证不占主线程
            time.sleep(0.1)
        UART_passer.loop_send_sentry = 0
    ind_sentry = (ind_sentry + 1) % 6

# 哨兵车间通信封装
def Referee_Transmit_Sentry(dataID, ReceiverId, datalength, x, y, targetId, ser):
    buffer = [0]
    buffer = buffer * 200
