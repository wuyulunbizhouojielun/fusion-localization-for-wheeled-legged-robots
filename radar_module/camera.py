...
def open_camera(camera_type, is_init, date, camera_brand):
    # try to open the camera
    cap = None
    init_flag = False
    try:
        if camera_brand == 0:
            cap = HT_Camera_MV(camera_type, is_init, date)
        elif camera_brand == 1:
            cap = HT_Camera_HK(camera_type, is_init, date)
        ...
        if not is_init:  # 若相机已经启动过一次则不进行调参
            ...
            win_loc = 0 if camera_type in [0,1,2] else 1
            # 移动至合适位置
            ...
            if key == ord('t') & 0xFF:
                cap.NoautoEx()  # 不自动曝光
                if camera_brand == 0:
                    tune_exposure_MV(cap)
                elif camera_brand == 1:
                    tune_exposure_HK(cap)
...
class Camera_Thread(object):
    def __init__(self, camera_type, camera_brand, video=False, video_path=None):
        '''
        ...
        self._is_init = False
        self._video = video
        self._camera_brand = camera_brand
        self._lock = Lock()
        if video_path is not None:
            self._video_path = video_path[camera_type]
        else:
    ...
    def open(self):
        ...
        if not self._video:
            if not self._open:
                self._open, self.cap = open_camera(self._camera_type, self._is_init, self._date, self._camera_brand)
...
# 5.2 海康相机和mindvision相机的参数定义范围不同，目前先分两个
def tune_exposure_HK(cap: HT_Camera_HK, high_fps=False):
    '''
    相机参数调节
...
def tune_exposure_MV(cap: HT_Camera_MV, high_fps=False):
    '''
    相机参数调节
    :param cap: camera target
    :param high_fps: 采用微秒/毫秒为单位调整曝光时间
